...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        result.append(dice())
        num_rolls -= 1
    return 1 if 1 in result else sum(result)
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    i, gcd, min_score = 1, 0, min(player_score, opponent_score)
    if min_score == 0: return False
    while i <= min_score:
        if (player_score % i == 0 and opponent_score % i == 0): gcd = i
        i += 1
    return False if gcd < 10 else True
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    return True if 3 > opponent_score - player_score > 0 else False
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score_gain = take_turn(player0_num_dice, score1, dice)
            score0 += score_gain
            if not extra_turn(score0, score1): who = other(who)
        else:
            player1_num_dice = strategy1(score1, score0)
            score_gain = take_turn(player1_num_dice, score0, dice)
            score1 += score_gain
            if not extra_turn(score1, score0): who = other(who)
    # END PROBLEM 5
    # (note that the indentation for the problem 6 prompt (***YOUR CODE HERE***) might be misleading)
    # BEGIN PROBLEM 6
        say = say(score0, score1) # makes the 'say' become the new commmentary function here
    # END PROBLEM 6
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        prev_high = running_high

        if who == 0: current = score0
        else: current = score1
        update_high = current - last_score

        if update_high > prev_high:
            prev_high = update_high
            print(update_high, "point(s)! The most yet for Player", who)

        return announce_highest(who, current, prev_high)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while i <= 10:
        ...
        if estimate_score > highest_score:
            num_rolls = i
            highest_score = estimate_score
        i += 1
    return num_rolls
    # -- Solution using array methods --
    # nums = [-1]
    # for i in range(1, 11):
    #     nums.append(make_averaged(roll_dice, trials_count)(i, dice))
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    return num_rolls if free_bacon(opponent_score) < cutoff else 0
    # END PROBLEM 10
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if extra_turn(free_bacon(opponent_score)+score, opponent_score): return 0
...
def final_strategy(score, opponent_score):
    ...
    This strategy applies extra_turn_strategy by default, and checks whether the
    score is more than 100 thereby using bacon_strategy instead.
    """
    # BEGIN PROBLEM 12
    score_after_bacon = score + free_bacon(opponent_score)
    if score_after_bacon >= 100: return bacon_strategy(score, opponent_score)
