...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    valid_paragraphs = [x for x in paragraphs if select(x)]
...
def about(topic):
    ...
    def select(paragraph):
        ...
        valid_paragraphs = [True if x in paragraph else False for x in topic]
        return any(valid_paragraphs) # True in valid_paragraphs
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3 
    k, total = 0, 0
    while k < min(len(typed_words), len(reference_words)):
        if typed_words[k] == reference_words[k]:
            total += 1
        k += 1
    return total / len(typed_words) * 100 if total != 0 else 0.0
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    return user_word if diff_function(user_word, possible_word, limit)> limit else possible_word
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    def helper(i, total):
        ...
        elif i == min(len(start), len(goal)):
            return total + abs(len(goal)-len(start))
        elif start[i] == goal[i]:
            return helper(i+1, total)
        elif start[i] != goal[i]:
            return helper(i+1, total+1)
    return helper(0, 0)
    # END PROBLEM 6
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0:
        ...
        diff = start[0] != goal[0] # True -> 1, False -> 0
...
def fastest_words(game):
    ...
    for i in word_indices:
        record = {p : time(game, p, i) for p in player_indices}
        fastest_words_list[min(record, key=record.get)].append(word_at(game, i))
