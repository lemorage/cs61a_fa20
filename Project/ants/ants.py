...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def find(place, curr_range):
            if place == beehive:
                return
            elif place.bees and self.min_range <= curr_range <= self.max_range:
                return rANTdom_else_none(place.bees)
            else:
                return find(place.entrance, curr_range+1)
        return find(self.place, 0)
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt will add more protection to our home base."""
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        def buff_reward(place):
            while place.exit is not None:
                place = place.exit
                if place.ant and not place.ant.buffed:
                    place.ant.damage *= 2
    ...
    def reduce_armor(self, amount):
        ...
        if not self.is_impostor and self.armor <= 0:
            bees_win()
        # END Problem EC.
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return False if self.place.ant is None or not self.place.ant.blocks_path else True
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.is_scared:
            destination = self.place if self.place.entrance is Hive else self.place.entrance
        # END EC
        if self.blocked():
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        return True if not self.contained_ant and not isinstance(other, ContainerAnt) else False
        # END Problem Optional 2
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        if target and target not in self.scared_bees:
            apply_status(make_scare, target, 2)
            self.scared_bees += [target]
        # END Problem Optional 4
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        attenuated_dmg = self.damage - (self.insects_shot * 0.05 + distance * 0.2)
        return max(0, attenuated_dmg)
        # END Problem Optional 5
