...
from inspect import ArgSpec  # Python 2 compatibility
...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        validate_procedure(operator := scheme_eval(first, env))
        # BEGIN PROBLEM 20
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_eval(expressions.first, env, scheme_nullp(expressions.rest)) # check for the TCO implementation
    else:
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is nil:
            ...
            return child.make_child_frame(formals.rest, vals.rest)
        # END PROBLEM 10
...
def do_if_form(expressions, env):
    ...
    if is_true_primitive(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True) # for the TCO implementation
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True) # ditto
...
def do_and_form(expressions, env):
    ...
    if scheme_nullp(expressions):
        return True
    elif is_false_primitive(val := scheme_eval(expressions.first, env)):
        return False
    else:
        return val if scheme_nullp(expressions.rest) else do_and_form(expressions.rest, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if scheme_nullp(expressions):
        return False
    elif is_true_primitive(val := scheme_eval(expressions.first, env, \
        scheme_nullp(expressions.rest))): # check for the TCO implementation
        return val
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        fbd = bindings.first
        validate_form(fbd, 1, 2)
        names = Pair(fbd.first, names)
        if fbd.rest:
            values = Pair(scheme_eval(fbd.rest.first, env), values)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 19
        # some other changes are required to be made to the optional third argument of
        # the scheme_eval function in the previous implementations.
