...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_b) if distance(city_b, city_c) < distance(city_a, city_c) else get_name(city_a)
...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
        return tree(label(t), [tree(t) for t in leaves])
    else:
...
def riffle(deck):
    ...
    """
    return [deck[(k % 2) * len(deck) // 2 + i // 2] for k in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    def build_result_branches(bs1, bs2):
        if not bs1: return bs2
        if not bs2: return bs1
        first = add_trees(bs1[0], bs2[0])
        return [first] + build_result_branches(bs1[1:], bs2[1:])
...
def build_successors_table(tokens):
    ...
    prev = '.'
    for word in tokens:
        if prev not in table: # table[prev] = [tokens[i+1] for i in range(len(tokens)-1) if prev == tokens[i]]
            table[prev] = []
        table[prev] += [word]
        prev = word
    return table
