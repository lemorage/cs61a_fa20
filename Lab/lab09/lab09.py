...
def subseqs(s):
    ...
    if not s:
        ...
        rest = [seq for seq in subseqs(s[1:])] # [[2], [3], [2, 3], []]
        return insert_into_all(s[0], rest) + rest # [[1], [1, 2], [1, 3], [1, 2, 3]] + ...
...
def trade(first, second):
    ...
    while not equal_prefix() and min(m, n) != min(len(first), len(second)):
