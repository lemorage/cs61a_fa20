...
def num_splits(s, d):
    ...
    def split(lst, diff):
        if not lst:
            ...
            with_first = split(lst[1:], diff + lst[0])
            without_first = split(lst[1:], diff - lst[0])
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if not check.deposited and self.holder is check.payable_to:
...
class Check(object):
    deposited = False
    def __init__(self, payable_to, balance):
...
def filterl(lst, pred):
    ...
    if not lst:
        ...
        return foldr(lst, lambda x, y: Link(x, y) if pred(x) else filterl(y, pred), Link.empty)
