...
def pingpong(n):
    ...
    True
    """
    def pingpong_helper(index, ppn, dir):
        if n == index: return ppn
        elif index % 8 == 0 or num_eights(index) != 0:
            return pingpong_helper(index+1, ppn-dir, -dir)
        else:
            return pingpong_helper(index+1, ppn+dir, dir)
...
def count_coins(total):
    ...
    True
    """
    def helper(goal, denom):
        if goal == 0: return 1
        elif goal < 0 or denom == None: return 0
        else:
            with_this_denom = helper(goal-denom, denom)
            without_this_denom = helper(goal, next_largest_coin(denom))
            return with_this_denom + without_this_denom
