...
def balanced(m):
    ...
    if is_planet(m):
        ...
        # make sure every left_torque == right_torque
        return balanced(end(left(m))) and balanced(end(right(m))) and\
        length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(y) > 0, AssertionError
...
def quadratic(x, a, b, c):
    ...
    def formula(t):
        return a*t*t + b*t + c
    lower_deri = 2 * a * lower_bound(x) + b
    upper_deri = 2 * a * upper_bound(x) + b
    extreme = -b / (2*a)
    if lower_deri >= 0 and upper_deri >= 0:
        domain = interval(formula(lower_bound(x)), formula(upper_bound(x)))
    elif lower_deri <= 0 and upper_deri <= 0:
        domain = interval(formula(upper_bound(x)), formula(lower_bound(x)))
    elif lower_deri >= 0 and upper_deri <= 0:
        domain = interval(formula(upper_bound(x)), formula(extreme))
    elif lower_deri <= 0 and upper_deri >= 0:
        domain = interval(formula(extreme), formula(lower_bound(x)))
    return domain
